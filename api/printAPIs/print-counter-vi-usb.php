...
//$tux = EscposImage::load(__DIR__."/codologo.png", false);   
//$printer->bitImage($tux);
  ...
  foreach ($items as $item) {
      $count++;
      $name_lines = str_split($item['name'], 35);
  ...
  $printer->setEmphasis(true);
  $printer -> text("TOTAL: $" . number_format((float)$order->total, 2, '.', '') . "\n");
  $printer->setEmphasis(false);
  $printer -> text("GST: $" . number_format((float)($order->total/11), 2, '.', '') . "\n");
